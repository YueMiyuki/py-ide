...
export default function PythonEditor() {
  ...
  const [versions, setVersions] = useState<Version[]>([{ id: 1, code: "print('Hello, World!')" }])
  const [currentVersion, setCurrentVersion] = useState<number>(1)
  const [isRunning, setIsRunning] = useState<boolean>(false)
  const [showDiff, setShowDiff] = useState<boolean>(false)
  const [diffVersions, setDiffVersions] = useState<{ old: number; new: number }>({ old: 0, new: 1 })
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#0a192f] text-white flex flex-col">
      ...
      >
        ...
        >
          ...
          <div className="flex space-x-2">
            ...
            <Button onClick={stopCode} disabled={!isRunning} variant="secondary">
              Stop
            </Button>
            <Button onClick={toggleDiff} variant="outline" className="bg-[#1a365d] text-white hover:bg-[#0d2a4a]">
              Show Diff
            </Button>
            <Button onClick={downloadCode} variant="outline" className="bg-[#1a365d] text-white hover:bg-[#0d2a4a]">
            ...
            <div className="flex-1 overflow-hidden rounded-lg shadow-lg">
              <Console
                ...
                onInput={(input) => {
                  ...
                  resetTimer() // Reset timer on user input
                }}
                ref={consoleRef}
              />
            </div>
      ...
      <footer className="relative z-10 py-4 text-center bg-[#0a192f] border-t border-[#1a365d]">
        <p>
          Designed with <span role="img" aria-label="love">❤️</span> by{' '}
          <Link href="https://mdesk.tech" target="_blank" rel="noopener noreferrer" className="text-[#ffa500] hover:underline">
            mdesk.tech
          </Link>
        </p>
      </footer>
        ...
        {showAlert && exitCode !== null && (
          ...
          >
            <Alert
              ...
              className={`border ${
                exitCode === 0 ? "bg-green-500 border-green-600" : "bg-red-500 border-red-600"
              } text-white`}
            >
              {exitCode === 0 ? (
