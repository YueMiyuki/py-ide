process.env.TZ = 'Asia/Hong_Kong'; // Set timezone to Hong Kong
...
io.on("connection", (socket) => {
  ...
  socket.on("run", (code) => {
    ...
    const dockerArgs = [
      ...
      'python:3.9-ide',
      'python', '-u', '/app/script.py', // Add -u flag for unbuffered output
    ]
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (runningProcess) {
        logWithTimestamp(
          `Timeout reached for ${clientIp} (container: exec-${processId})`
        );

        // Force kill the container using docker kill
        const killer = spawn("docker", ["kill", `exec-${processId}`]);

        killer.on("close", (code) => {
          if (code === 0) {
            logWithTimestamp(`Container exec-${processId} killed successfully`);
          } else {
            logWithTimestamp(`Failed to kill container exec-${processId}`);
          }
        });
    ...
    pythonProcess.on("close", (code) => {
      ...
      if (runningProcess) {
        logWithTimestamp(
          `Process ${processId} exited with code ${code} (${clientIp})`
        );
  ...
  socket.on("stop", () => {
    ...
    if (runningProcess) {
      logWithTimestamp(
        `Manual stop for ${runningProcess.clientIp} (process: ${runningProcess.id})`
      );
      ...
      try {
        fs.unlinkSync(runningProcess.filePath);
        logWithTimestamp(
          `Cleaned temp file after manual stop: ${runningProcess.filePath}`
        );
      } catch (err) {
  ...
  socket.on("disconnect", () => {
    const runningProcess = runningProcesses.get(socket.id);
    logWithTimestamp(
      `Disconnect from ${clientIp} ${runningProcess ? "(process running)" : ""}`
    );
    ...
    if (runningProcess) {
      ...
      try {
        fs.unlinkSync(runningProcess.filePath);
        logWithTimestamp(
          `Cleaned temp file after disconnect: ${runningProcess.filePath}`
        );
      } catch (err) {
...
server.listen(PORT, () => {
  ...
  logWithTimestamp(
    `Container resource limits: ${JSON.stringify(DOCKER_RESOURCE_LIMITS)}`
  );
});
